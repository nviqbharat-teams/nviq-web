...
export default function Footer() {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      setSubmitted(true);
      trackEvent("form_submit", "newsletter-subscribe", `Subscribed to newsletter: ${email.trim()}`);
      setEmail("");
    } else {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 pb-10 border-b border-white/10">
          ...
          <div className="md:col-span-1">
            ...
            <div className="mt-6 flex flex-col gap-2.5 text-sm text-gray-400">
              <a href="tel:+919694551326" className="flex items-center gap-2 hover:text-white transition-colors">
              ...
              {submitted ? (
                ...
                <form onSubmit={handleSubscribe} className="flex flex-col gap-2">
                  <div className="flex gap-2">
                    ...
                    <button
                      type="submit"
                      className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors cursor-pointer disabled:opacity-50"
                      disabled={loading}
                    >
                      {loading ? "..." : "Subscribe"}
                    </button>
